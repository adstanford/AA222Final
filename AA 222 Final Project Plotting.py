#from project2 import pquad, basis, isfeasible, cross_entropy_method
from AA_222_Final_Project_Code import pquad, basis
...
def cross_entropy_method(f, P, c, rho, k_max, l, m=30, m_elite=15):
    ...
    for k in range(k_max):
        ...
        #sigma = np.diag(np.diag(np.cov(elite_samples, rowvar=False)))
        #sigma += np.diag(0.1*np.ones(m_elite)) 
...
Optimums1 = optimize_Simplex(f, c, np.array([0,0]), 200)
X = Optimums1[:,0]
Y = Optimums1[:,1]

plt.plot(X,Y, color = "blue", label = 'One Year')

Optimums1 = optimize_Simplex(f_2, c, np.array([0,0]), 200)
X = Optimums1[:,0]
Y = Optimums1[:,1]
...
Optimums1 = optimize_Simplex(f, c5, np.array([0,0,0,0,0]), 200)
fun = np.array([])
for val in Optimums1:
    fun = np.append(fun,f(val))
...
Optimums1 = optimize_Simplex(f_2, c5, np.array([0,0,0,0,0]), 200)
fun = np.array([])
for val in Optimums1:
    fun = np.append(fun,f_2(val))
